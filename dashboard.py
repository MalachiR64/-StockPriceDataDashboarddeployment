...
# Connect to Azure Blob Storage using Streamlit secrets
@st.cache_resource
def get_blob_service_client():
    try:
        # Get connection string from Streamlit secrets
        connect_str = st.secrets["CONNECT_STR_BLOB"]
        blob_service_client = BlobServiceClient.from_connection_string(connect_str)
        return blob_service_client
    except KeyError:
        st.error("Azure Blob Storage connection string not found in secrets. Please configure CONNECT_STR_BLOB in your Streamlit secrets.")
        return None
...
# Get container name from Streamlit secrets
def get_container_name():
    try:
        container_name = st.secrets.get("CONTAINER_NAME", "stockmarket")  # Default to "stockmarket" if not set
        return container_name
    except:
        return "stockmarket"  # Fallback default
...
if stocks_df is None:
    st.error("Failed to load stocks data. Please check your Azure configuration.")
    st.stop()
...
if sectors_df is None:
    st.warning("Sectors data not available.")
    sectors_df = pd.DataFrame()

if hist_stocks_df is None:
    st.warning("Historical stocks data not available.")
    hist_stocks_df = pd.DataFrame()
...
with tab2:
    ...
    if search_term:
        filtered_stocks = filtered_stocks[
            filtered_stocks['stock_symbol'].str.contains(search_term, case=False, na=False) | 
            filtered_stocks['stock_name'].str.contains(search_term, case=False, na=False)
        ]
